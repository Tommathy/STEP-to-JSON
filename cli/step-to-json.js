...
const relationsBar = new cliProgress.SingleBar({
    format:
        'Parsing relations \t\t|' +
        colors.cyan('{bar}') +
        '| {percentage}% || {value}/{total} Relations parsed',
    barCompleteChar: '\u2588',
    barIncompleteChar: '\u2591',
    hideCursor: true
});
...
const relations = parser.parseNextAssemblyUsageOccurences(
    preprocessedObject.data.nextAssemblyUsageOccurences,
    assemblyUsageSubject
);
...
const productBar = new cliProgress.SingleBar({
    format:
        'Parsing products \t\t|' +
        colors.cyan('{bar}') +
        '| {percentage}% || {value}/{total} Products parsed',
    barCompleteChar: '\u2588',
    barIncompleteChar: '\u2591',
    hideCursor: true
});
...
const products = parser.parseProductDefinitions(
    preprocessedObject.data.productDefinitions,
    productDefinitionSubject
);
...
const buildBar = new cliProgress.SingleBar({
    format:
        'Building the output \t\t|' +
        colors.cyan('{bar}') +
        '| {percentage}% || {value}/{total} Relations analyzed',
    barCompleteChar: '\u2588',
    barIncompleteChar: '\u2591',
    hideCursor: true
});
